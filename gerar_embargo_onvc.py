'''
Script para gerar os laudos de embargo cautelar. Ele usa a tabela gerada pelo código "gerar_tabela_usada_no_layout" como um dos inputs
...
cpf_folder = r"Output"
id_embargo_folder = Path(r"\\Bp-1hd57t3-inea\e\COGET\INPUTS_SCRIPTS")
ref_folder = r"arquivos"

# Planilhas de input, a gerada pelo script e a de referências
tabela_cpf = os.path.join(cpf_folder, "Planilha_ONVC_CAR.xlsx")
tabela_dissolve = os.path.join(csv_folder, "tabela_Dissolve_nova_camada_inter_GPL_08_Codigo_CAR.csv")
...
layout_folder = os.path.join(output_folder, "laudos_embargo_onvc")
...
def addPageNumber(canvas, doc, id_embargo_onvc_for_report):
    '''
    ...
    report_text = f"Laudo {id_embargo_onvc_for_report}"
...
def plot_map_for_idtxt_satellite(gdf, idtxt, png_folder, buffer_distance=19, color='none', opacity=0.9):
    ...
    logging.info(f"ID encontrado no shapefile: {idtxt}")
    ...
    logging.info(f"Salvando o mapa em: {output_path}")
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()


# Função para gerar o PDF para cada `idtxt`
def create_pdf_for_idtxt(idtxt, nome_table, cicatriz_data, inter_data, png_folder, cntd, link_poligono, nome_car, current_idtxt, id_embargo_onvc):
    '''
    ...
    '''
    pdf_file = os.path.join(layout_folder, f"{idtxt}_{nome_car}.pdf")
    
    # Definindo a função wrapper para passar o ID do laudo ao rodapé
    id_embargo_onvc_for_report = f"{id_embargo_onvc}"
    def add_page_num_wrapper(canvas, doc):
        addPageNumber(canvas, doc, id_embargo_onvc_for_report)
    ...
    # Adiciona o título
    elements.append(Paragraph(f"Laudo de Embargo Cautelar", styles['Titulo']))
    elements.append(Spacer(0, 0 * cm))

    # Adiciona o subtítulo
    subtitulo = "Documento automatizado gerado a partir do sistema de detecção de alertas de cicatrizes"
    ...
    CAR_DF = pd.read_excel(tabela_cpf)
    mapa_info = CAR_DF.rename(columns={'idtxt': 'ID da Cicatriz', 'centro_x': 'Latitude (centro)',
    ...
    # Informações do CAR
    car_info = pd.DataFrame({'Código do CAR': [nome_car]})
    add_table_with_split(car_info, "Informações do CAR", 1, [100])
    info_car = nome_table[['Área do Imóvel (ha)', 'Classificação Fundiária', 'Módulo Fiscal',  'Fração Mínima de Parcelamento (ha)']]
    add_table_without_space(info_car, 2, [50, 50])
    elements.append(Spacer(1, 0.5 * cm))   
    ...
    if os.path.exists(png_path):
        ...
        logging.error(f"Imagem PNG do mapa não encontrada para ID: {idtxt}")
    ...
    conclusao = """A supressão de vegetação detectada na Cicatriz em epígrafe configura queimada e/ou incêndio florestal 
    ...
    logging.info(f"✅  {cntd} PDFs gerados, Alerta: {idtxt}, CAR: {nome_car}\n")
...
nome_table = pd.read_csv(tabela_dissolve, delimiter=';', encoding='latin1')
nome_table = nome_table.rename(columns={
    'idtxt': 'ID da Cicatriz', 'cod_imovel': 'Código do CAR', 'mod_fiscal': 'Módulo Fiscal', 'area_prop': 'Área do Imóvel (ha)',
    'class_fund': 'Classificação Fundiária'
})[['ID da Cicatriz', 'Código do CAR', 'Módulo Fiscal', 'Área do Imóvel (ha)', 'Classificação Fundiária']]

# 1. Substitui a vírgula por ponto (convertendo para string primeiro para garantir)
nome_table['Área do Imóvel (ha)'] = nome_table['Área do Imóvel (ha)'].astype(str).str.replace(',', '.')
nome_table['Módulo Fiscal'] = nome_table['Módulo Fiscal'].astype(str).str.replace(',', '.')

# 2. Agora sim, converte para numérico sem o argumento 'decimal'
nome_table['Área do Imóvel (ha)'] = pd.to_numeric(nome_table['Área do Imóvel (ha)'], errors='coerce')
nome_table['Módulo Fiscal'] = pd.to_numeric(nome_table['Módulo Fiscal'], errors='coerce')

# Ajustando casas decimais e visualização (Seu código original mantido aqui)
nome_table['Área do Imóvel (ha)'] = nome_table['Área do Imóvel (ha)'].apply(
    lambda x: f"{x:.4f}" if pd.notna(x) else ''
)
nome_table['Módulo Fiscal'] = nome_table['Módulo Fiscal'].apply(
    lambda x: f"{x:.2f}" if pd.notna(x) else ''
)

# Cria coluna fmp com valores iguais a 2
nome_table['Fração Mínima de Parcelamento (ha)'] = 2
...
try:
    ...
    # NOVO: Carregar tabela de mapeamento ID Alerta -> ID Laudo (ASSUMINDO NOME df_id_embargo_ONVC.xlsx)
    df_id_embargo_ONVC_path = id_embargo_folder / "df_id_embargo_ONVC.xlsx"
    df_id_embargo_ONVC = pd.read_excel(df_id_embargo_ONVC_path)
    ...
    for id in lista_ids:
        pao = 'ONVC' + str(id)
        lista_formatada.append(pao)

    # Loop que percorre cada id
    for _, row in nome_table.iterrows():
        # Captura as variaveis necessárias:
        current_idtxt = row['ID da Cicatriz']
        nome_car = row['Código do CAR']
        idtxt = str(current_idtxt)
        
        if ('ONVC' + idtxt) not in lista_formatada: # Se um ID não estiver na lista (com prefixo ONVC), ele é pulado
            continue

        # 1. Filtra pela combinação ID da Cicatriz e Código do CAR (cod_imovel)
        laudo_row = df_id_embargo_ONVC[
            (df_id_embargo_ONVC['idtxt'].astype(str) == idtxt) &
            (df_id_embargo_ONVC['cod_imovel'].astype(str) == nome_car)
        ]
        
        # 2. Atribui o id_embargo_onvc
        if not laudo_row.empty and 'id_embargo_onvc' in laudo_row.columns:
            id_embargo_onvc = laudo_row['id_embargo_onvc'].iloc[0]
        else:
            id_embargo_onvc = f"Não Informado ({idtxt}-{nome_car})"
        ...
        try: # Gerar mapa para o idtxt (usando geometria do shapefile)
            ...
            logging.info(f"⏳🗺️  Criando os mapas da cicatriz para o ID {current_idtxt}")

        except Exception as e:
            logging.error(f"Erro ao gerar a imagem no ID {current_idtxt}: {str(e)}")

        # Filtrando os dados de cicatriz e interseção
        cicatriz_df[id_column] = cicatriz_df[id_column].astype(str) # transforma o tipo da coluna de id em texto
        id_form = str(idtxt)[:4]
        ...
        if cicatriz_filtered.empty:
            ...
            continue 
        nome_table_filtered = nome_table[nome_table['ID da Cicatriz'].astype(str) == id]
        nome_table_filtered = nome_table_filtered[nome_table_filtered['Código do CAR'].astype(str) == nome_car]
        ...
        if valid_inter_dfs:
            ...
            try:
                ...
                logging.error(f"Ocorreu um erro durante tratar intersect table: {e}")        
        try: # Geração do PDF para o id, car atual e por nome
            logging.info(f"⏳  Gerando layout para: ID {current_idtxt}, CAR {nome_car}")
            create_pdf_for_idtxt(idtxt, nome_table_filtered, cicatriz_filtered, inter_filtered, png_folder, cntd, link_poligono, nome_car, current_idtxt, id_embargo_onvc)
        except Exception as e:
...
delete_files(mapas_folder, "APAGAR")
